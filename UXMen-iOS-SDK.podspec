#
# Be sure to run `pod lib lint UXMen-iOS-SDK.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see https://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'UXMen-iOS-SDK'
  s.version          = '0.1.9'
  s.summary          = 'UXMen iOS SDK provides user actions as projection of flow chart.'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
UXMen iOS SDK provides user actions as projection for determine how user reacts to UX style of your mobile application. You can optimize your UI/UX according to your own user data.
                       DESC

  s.homepage         = 'https://github.com/git-erdinc-bakir/UXMen-iOS-SDK'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'git-erdinc-bakir' => 'e.bakir@thy.com' }
  s.source           = { :git => 'https://github.com/git-erdinc-bakir/UXMen-iOS-SDK.git', :tag => s.version.to_s }
  # s.social_media_url = 'https://twitter.com/<TWITTER_USERNAME>'

  s.ios.deployment_target = '8.0'

  s.source_files = 'UXMen-iOS-SDK/Classes/**/*'
  
  # s.resource_bundles = {
  #   'UXMen-iOS-SDK' => ['UXMen-iOS-SDK/Assets/*.png']
  # }

  # s.public_header_files = 'Pod/Classes/**/*.h'
  # s.frameworks = 'UIKit', 'MapKit'
  # s.dependency 'AFNetworking', '~> 2.3'
end
